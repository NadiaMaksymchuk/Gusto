...
export const createCourierValidator = [
  ...
  body("vehicleNumber")
    ...
    .not()
    .isEmpty()
    .withMessage("Vehicle number must be a non-empty string"),
  body("availabilityStatus")
    .isNumeric()
    .not()
    .isEmpty()
    .withMessage("Availability status must be a non-empty number"),
  body("sex")
    .isNumeric()
    .not()
    .isEmpty()
    .withMessage("Sex must be a non-empty number"),
  body("password")
    .isString()
    .isLength({ min: 6 })
