...
export class OrdersController {
  ...
  createOrder = async (req: Request, res: Response) => {
    ...
    newOrder.orderDate = new Date().toISOString().slice(0, 19).replace('T', ' ');
  ...
  getOrdersWithOrderItemsAndImagesByUserAndStatus = async (req: Request, res: Response) => {
    const userId = +req.params.userId;
    const orderStatus = +req.params.orderStatus;

    const ordersWithItemsAndImages = await this.ordersRepository.getOrdersWithOrderItemsAndImagesByUserAndStatus(
      userId,
      orderStatus
    );
    ...
    return ResponseHandler.success<OrderWithItemsImagesAndRestaurantDto[]>(res, ordersWithItemsAndImages, "Orders found");
  }
  ...
  updateOrderStatus = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return ResponseHandler.error(res, `Error in updating order status: ${err}`);
    }
  };

  updateDeliveryTime = async (req: Request, res: Response) => {
    const orderId = +req.params.id;
    const newDeliveryTime = new Date(req.body.deliveryTime).toISOString().slice(0, 19).replace('T', ' ');
    ...
    try {
      ...
      return ResponseHandler.error(res, `Error in updating delivery time: ${err}`);
    }
  };
}
