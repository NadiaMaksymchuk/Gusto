...
export class ChatsController {
  ...
  getNumberOfUnreadMessages = async (req: Request, res: Response) => {
    ...
    try {
      ...
      ResponseHandler.success<number>(res, unreadMessageCount, 'Chats retrieved');
    } catch (err) {
  ...
  getChatsByUserId = async (req: Request, res: Response) => {
    try {
      ...
      ResponseHandler.error(res, `Error in retrieving unread messages count: ${err}`);
    }
  }
  ...
  getFirst30MessagesByChatId = async (req: Request, res: Response) =>  {
    ...
    try {
      const chatPersons = await this.messagesRepository.getFirst30MessagesByChatId(chatId);
      ResponseHandler.success(res, chatPersons, 'Messages retrieved');
    } catch (err) {
      ResponseHandler.error(res, `Error in retrieving unread and last messages: ${err}`);
    }
  }
  ...
  getUnreadAndLastMessagesAsync = async (req: Request, res: Response) => {
    ...
    try {
      ...
      ResponseHandler.success(res, chatPersons, 'Unread and last messages retrieved');
    } catch (err) {
      ResponseHandler.error(res, `Error in retrieving unread and last messages: ${err}`);
    }
  }
