...
export class UserService implements IUserService {
  ...
  async getAll(): Promise<ApiResponse<UserDto[]>> {
    ...
    const response = new ApiResponse(HttpStatusCode.OK, users, 'Data retrieved successfully')
    return response
  }
  ...
  async getUserByEmail(email: string): Promise<ApiResponse<UserDto>> {
    ...
    if (Object.keys(user).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `User by email ${email} not found`)
    }
  ...
  async signUp(newUser: CreateUserDto) {
    ...
    if (Object.keys(user).length !== 0) {
      return new ApiResponse(HttpStatusCode.BadRequest, null, `User by email ${newUser.email} alredy exist`)
    }
    ...
    const { token } = encodeSession('mysecretkey', partialSession)
  ...
  async signIn(loginUserDto: LoginUserDto) {
    ...
    if (Object.keys(user).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `User by email ${loginUserDto.email} not found`)
    }
  ...
  async updateUser(userId: number, updatedUserData: UpdateUserDto) {
    ...
    if (Object.keys(user).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `User by id ${userId} not found`)
    }
