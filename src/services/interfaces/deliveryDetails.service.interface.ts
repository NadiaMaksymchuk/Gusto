...
export interface IDeliveryDetailsService {
    createDeliveryDetail(newDeliveryDetail: CreateDeliveryDetailDto): Promise<ApiResponse<void>>;
    deleteDeliveryDetail(deliveryDetailId: number): Promise<ApiResponse<void>>;
    getDeliveryDetailsByCourierId(courierId: number): Promise<ApiResponse<DeliveryDetailWithCourierOrderRestaurantAndOrderItemsDto[]>>;
    getDeliveryDetailsByOrderId(orderId: number): Promise<ApiResponse<DeliveryDetailWithCourierAndOrderDto[]>>;
  }