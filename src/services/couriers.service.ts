...
export class CouriersService implements ICouriersService {
  ...
  async signUp(newCourier: CreateCourierDto) {
    ...
    if (Object.keys(courier).length !== 0) {
      return new ApiResponse(HttpStatusCode.BadRequest, null, `Courier by email ${newCourier.email} alredy exist`)
    }
    ...
    const { token } = encodeSession('secretkey', partialSession)

    return new ApiResponse(HttpStatusCode.Created, token, 'Courier created')
  }

  async signIn(loginCourierDto: LoginCourierDto) {
    const courier = await this.couriersRepository.getCourierByEmail(loginCourierDto.email)

    if (Object.keys(courier).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `Courier by email ${loginCourierDto.email} not found`)
    }

    if (!(await verifyPassword(loginCourierDto.password, courier.salt, courier.password))) {
      return new ApiResponse(HttpStatusCode.BadRequest, null, `Invalid password`)
    }
  ...
  async getAllCouriers(): Promise<ApiResponse<CourierDto[]>> {
    ...
    return new ApiResponse(HttpStatusCode.OK, couriers, 'Couriers retrieved successfully')
  }

  async getCourierById(courierId: number): Promise<ApiResponse<CourierDto | null>> {
    const courier = await this.couriersRepository.getCourierById(courierId)

    if (Object.keys(courier).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `Courier by id ${courierId} not found`)
    }

    return new ApiResponse(HttpStatusCode.OK, courier, 'Courier retrieved successfully')
  }

  async getCouriersByAvailabilityStatus(status: number): Promise<ApiResponse<CourierDto[]>> {
    const couriers = await this.couriersRepository.getCouriersByAvailabilityStatus(status)
    return new ApiResponse(HttpStatusCode.OK, couriers, 'Couriers retrieved successfully')
  }

  async updateCourier(courierId: number, updatedCourierData: UpdateCourierDto): Promise<ApiResponse<void>> {
    const courier = await this.couriersRepository.getCourierById(courierId)
    ...
    return new ApiResponse(HttpStatusCode.OK, null, 'Courier updated successfully')
  }

  async setAvailabilityStatus(courierId: number, availabilityStatus: number): Promise<ApiResponse<void>> {
    const courier = await this.couriersRepository.getCourierById(courierId)
    ...
    return new ApiResponse(HttpStatusCode.OK, null, 'Courier availability status updated successfully')
  }
  ...
  async deleteCourier(courierId: number): Promise<ApiResponse<void>> {
    ...
    return new ApiResponse(HttpStatusCode.NoContent, null, 'Courier deleted successfully')
  }
}
