...
export class CouriersService implements ICouriersService{
    ...
    async signUp(newCourier: CreateCourierDto) {
      ...
      async signIn(loginCourierDto: LoginCourierDto) {
        let courier = await this.couriersRepository.getCourierByEmail(loginCourierDto.email);
        ...
        if (
          !(await verifyPassword(loginCourierDto.password, courier.salt, courier.password))
        ) {
          return new ApiResponse(
            HttpStatusCode.BadRequest,
            null,
      ...
      async getCouriersByAvailabilityStatus(
        ...
        const couriers = await this.couriersRepository.getCouriersByAvailabilityStatus(
          status
        );
        return new ApiResponse(
          HttpStatusCode.OK,
