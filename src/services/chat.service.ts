...
export class ChatService implements IChatService {
  ...
  async getChatById(chatId: number): Promise<ApiResponse<ChatDto | null>> {
    ...
    if (Object.keys(chat).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `Chat by id ${chatId} not found`)
    }
  ...
  async updateMessage(messageId: number, text: string): Promise<ApiResponse<void>> {
    ...
    if (Object.keys(message).length === 0) {
      return new ApiResponse(HttpStatusCode.NotFound, null, `Message by id ${messageId} not found`)
    }
