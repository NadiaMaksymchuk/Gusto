...
class DeliveryDetailsService implements IDeliveryDetailsService {
  ...
  async createDeliveryDetail(newDeliveryDetail: CreateDeliveryDetailDto): Promise<ApiResponse<void>> {
    await this.deliveryDetailsRepository.createDeliveryDetail(newDeliveryDetail)
    return new ApiResponse(HttpStatusCode.Created, null, 'Delivery detail created successfully')
  }

  async deleteDeliveryDetail(deliveryDetailId: number): Promise<ApiResponse<void>> {
    await this.deliveryDetailsRepository.deleteDeliveryDetail(deliveryDetailId)
    return new ApiResponse(HttpStatusCode.NoContent, null, 'Delivery detail deleted successfully')
  }
  ...
  async getDeliveryDetailsByCourierId(
    ...
    return new ApiResponse(HttpStatusCode.OK, deliveryDetails, 'Delivery details retrieved successfully')
  }

  async getDeliveryDetailsByOrderId(orderId: number): Promise<ApiResponse<DeliveryDetailWithCourierAndOrderDto[]>> {
