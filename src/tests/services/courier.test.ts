...
container
  ...
  .toConstantValue(mockCouriersRepository);
...
describe("sign up", () => {
  ...
  test("should send a status code of 400 when courier exists", async () => {
    ...
    ).mockResolvedValueOnce(existingCourierDto);
    ...
    expect(response.message).toBe(
      `Courier by email ${createCourierDto.email} alredy exist`,
    );
  });
...
describe("getAllCouriers", () => {
  ...
  test("should return all couriers", async () => {
    ...
    (mockCouriersRepository.getAllCouriers as jest.Mock).mockResolvedValueOnce(
      expectedCouriers,
    );
...
describe("getCourierById", () => {
  ...
  test("should return 404 for non-existent courier by id", async () => {
    ...
    expect(response.data).toBeNull();
    expect(response.message).toBe(
      `Courier by id ${nonExistentCourierId} not found`,
    );
  });
});
...
describe("updateCourier", () => {
  ...
  test("should update a courier successfully", async () => {
    ...
    expect(mockCouriersRepository.updateCourier).toHaveBeenCalledWith(
      courierId,
      updatedCourierData,
    );
  });
...
describe("setAvailabilityStatus", () => {
  ...
  test("should set availability status for a courier successfully", async () => {
    ...
    const response = await courierService.setAvailabilityStatus(
      courierId,
      availabilityStatus,
    );
    ...
    expect(mockCouriersRepository.setAvailabilityStatus).toHaveBeenCalledWith(
      courierId,
      availabilityStatus,
    );
  });
...
describe("deleteCourier", () => {
  ...
  test("should delete a courier successfully", async () => {
    ...
    expect(mockCouriersRepository.deleteCourier).toHaveBeenCalledWith(
      courierId,
    );
  });
