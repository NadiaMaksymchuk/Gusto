...
container.bind<IOrdersRepository>('IOrdersRepository').toConstantValue(mockOrdersRepository);
...
describe('OrderService', () => {
    ...
    test('getOrdersWithOrderItemsAndImagesByUserAndStatus should return ApiResponse with status 200', async () => {
        ...
        mockOrdersRepository.getOrdersWithOrderItemsAndImagesByUserAndStatus.mockResolvedValueOnce(mockOrders);

        const response = await orderService.getOrdersWithOrderItemsAndImagesByUserAndStatus(userId, orderStatus);
    ...
    test('updateOrderStatus should return ApiResponse with status 204 for existing order', async () => {
        const orderId = 1;
        const newStatus = 3;
        ...
        expect(mockOrdersRepository.updateOrderStatus).toHaveBeenCalledWith(orderId, newStatus);
    });
    ...
    test('updateDeliveryTime should return ApiResponse with status 204 for existing order', async () => {
        const orderId = 1;
        ...
        const response = await orderService.updateDeliveryTime(orderId, newDeliveryTime);
        ...
        expect(mockOrdersRepository.updateDeliveryTime).toHaveBeenCalledWith(orderId, newDeliveryTime);
    });
