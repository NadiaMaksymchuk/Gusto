...
container
  ...
  .toConstantValue(mockNotificationsRepository);
...
describe("NotificationsService", () => {
  ...
  test("createNotification should return ApiResponse with status 201", async () => {
    ...
    expect(
      mockNotificationsRepository.createNotification,
  ...
  test("deleteNotification should return ApiResponse with status 204", async () => {
    ...
    expect(
      mockNotificationsRepository.deleteNotification,
    ).toHaveBeenCalledTimes(1);
    expect(mockNotificationsRepository.deleteNotification).toHaveBeenCalledWith(
      notificationId,
    );
  });
  ...
  test("getUnreadNotificationsByUserId should return ApiResponse with status 200", async () => {
    ...
    mockNotificationsRepository.getUnreadNotificationsByUserId.mockResolvedValueOnce(
      mockNotifications,
    );

    const response =
      await notificationsService.getUnreadNotificationsByUserId();
  ...
  test("readNotificationStatus should return ApiResponse with status 200", async () => {
    ...
    expect(
      mockNotificationsRepository.readNotificationStatus,
    ).toHaveBeenCalledTimes(1);
    expect(
      mockNotificationsRepository.readNotificationStatus,
