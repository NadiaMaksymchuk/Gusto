...
container
  .bind<IOrderService>("IOrderService")
  .toConstantValue(mockOrderService);
...
describe("order controller", () => {
  ...
  test("getOrdersWithOrderItemsAndImagesByUserAndStatus should return ApiResponse with status 200", async () => {
    ...
    (
      mockOrderService.getOrdersWithOrderItemsAndImagesByUserAndStatus as jest.Mock
    ...
    await orderController.getOrdersWithOrderItemsAndImagesByUserAndStatus(
      mockRequest,
      mockResponse,
    );
    ...
    expect(
      mockOrderService.getOrdersWithOrderItemsAndImagesByUserAndStatus,
    ).toHaveBeenCalledWith(userId, orderStatus);
  });
  ...
  test("updateOrderStatus should return ApiResponse with status 200 for existing order", async () => {
    ...
    (mockOrderService.updateOrderStatus as jest.Mock).mockResolvedValueOnce(
      response,
    );
  ...
  test("updateDeliveryTime should return ApiResponse with status 200 for existing order", async () => {
    ...
    (mockOrderService.updateDeliveryTime as jest.Mock).mockResolvedValueOnce(
      response,
    );
    ...
    expect(mockResponse.status).toHaveBeenCalledWith(HttpStatusCode.OK);
    expect(mockResponse.json).toHaveBeenCalledWith(response);
    expect(mockOrderService.updateDeliveryTime).toHaveBeenCalledWith(
      orderId,
      newDeliveryTime,
    );
  });
