...
container.bind<INotificationsService>('INotificationsService').toConstantValue(mockNotificationService)
...
describe('notification controller', () => {
  ...
  test('createNotification should return ApiResponse with status 201', async () => {
    ...
    const response = new ApiResponse(HttpStatusCode.Created, null, 'Notification created successfully')

    ;(mockNotificationService.createNotification as jest.Mock).mockResolvedValueOnce(response)
  ...
  test('deleteNotification should return ApiResponse with status 204', async () => {
    ...
    const response = new ApiResponse(HttpStatusCode.NoContent, null, 'Notification deleted successfully')
    ;(mockNotificationService.deleteNotification as jest.Mock).mockResolvedValueOnce(response)
  ...
  test('getUnreadNotificationsByUserId should return ApiResponse with status 200', async () => {
    ...
    ;(mockNotificationService.getUnreadNotificationsByUserId as jest.Mock).mockResolvedValueOnce(response)
    ...
    await notificationController.getUnreadNotificationsByUserId(mockRequest, mockResponse)
  ...
  test('readNotificationStatus should return ApiResponse with status 200', async () => {
    ...
    const response = new ApiResponse(HttpStatusCode.OK, null, 'Notification status updated successfully')
    ;(mockNotificationService.readNotificationStatus as jest.Mock).mockResolvedValueOnce(response)
    ...
    await notificationController.readNotificationStatus(mockRequest, mockResponse)
