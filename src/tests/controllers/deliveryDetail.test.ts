...
container.bind<IDeliveryDetailsService>('IDeliveryDetailsService').toConstantValue(mockDeliveryDetailsService)
...
describe('delivery details controller', () => {
  ...
  test('createDeliveryDetail should return ApiResponse with status 201', async () => {
    ...
    const mockApiResponse = new ApiResponse(HttpStatusCode.Created, null, 'Delivery detail created successfully')
    ;(mockDeliveryDetailsService.createDeliveryDetail as jest.Mock).mockResolvedValueOnce(mockApiResponse)

    await deliveryDetailsController.createDeliveryDetail(mockRequest, mockResponse)
    ...
    expect(mockDeliveryDetailsService.createDeliveryDetail).toHaveBeenCalledWith(mockCreateDeliveryDetailDto)
  })

  test('deleteDeliveryDetail should return ApiResponse with status 204', async () => {
    const deliveryDetailId = 1

    const mockApiResponse = new ApiResponse(HttpStatusCode.NoContent, null, 'Delivery detail deleted successfully')
    ;(mockDeliveryDetailsService.deleteDeliveryDetail as jest.Mock).mockResolvedValueOnce(mockApiResponse)
    ...
    await deliveryDetailsController.deleteDeliveryDetail(mockRequest, mockResponse)

    expect(mockResponse.status).toHaveBeenCalledWith(HttpStatusCode.NoContent)
    expect(mockResponse.json).toHaveBeenCalledWith(mockApiResponse)
    expect(mockDeliveryDetailsService.deleteDeliveryDetail).toHaveBeenCalledWith(deliveryDetailId)
  })
  ...
  test('getDeliveryDetailsByCourierId should return ApiResponse with status 200', async () => {
    ...
    ;(mockDeliveryDetailsService.getDeliveryDetailsByCourierId as jest.Mock).mockResolvedValueOnce(mockApiResponse)

    await deliveryDetailsController.getDeliveryDetailsByCourierId(mockRequest, mockResponse)

    expect(mockResponse.status).toHaveBeenCalledWith(HttpStatusCode.OK)
    expect(mockResponse.json).toHaveBeenCalledWith(mockApiResponse)
    expect(mockDeliveryDetailsService.getDeliveryDetailsByCourierId).toHaveBeenCalledWith(courierId)
  })
})
