...
class CouriersRepository implements ICouriersRepository {
  ...
  async getCourierByEmail(email: string): Promise<CourierDto> {
    return new Promise((resolve, reject) => {
      sqlPool.query(`SELECT * FROM Couriers WHERE email = "${email}";`, function (err, res) {
        if (err) {
          reject(err)
        }
  ...
  async getAllCouriers(): Promise<CourierDto[]> {
    return new Promise((resolve, reject) => {
      sqlPool.query('SELECT * FROM Couriers;', function (err, res) {
        if (err) {
          reject(err)
        }
  ...
  async getCourierById(courierId: number): Promise<CourierDto | null> {
    return new Promise((resolve, reject) => {
      sqlPool.query(`SELECT * FROM Couriers WHERE id = ${courierId};`, function (err, res) {
        if (err) {
          reject(err)
        }
  ...
  async getCouriersByAvailabilityStatus(status: number): Promise<CourierDto[]> {
    return new Promise((resolve, reject) => {
      sqlPool.query(`SELECT * FROM Couriers WHERE availabilityStatus = ${status};`, function (err, res) {
        if (err) {
          reject(err)
        }
