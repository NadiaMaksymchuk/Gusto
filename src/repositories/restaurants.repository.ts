...
class RestaurantsRepository {
  async createRestaurant(newRestaurant: CreateRestaurantDto) {
    const values = [
        ...Object.values(newRestaurant),
    ];

    const queryText = `INSERT INTO Restaurants (name, cuisineType, address, contacts) VALUES (${arrayToStringWithQuotes(values)});`;
  ...
  async getAllRestaurants(): Promise<RestaurantDto[]> {
    return new Promise((resolve, reject) => {
      sqlPool.query("SELECT * FROM Restaurants;", function (err: any, res: any) {
        if (err) {
          reject(err);
        }
  ...
  async getRestaurantById(restaurantId: number): Promise<RestaurantDto | null> {
    return new Promise((resolve, reject) => {
      sqlPool.query(`SELECT * FROM Restaurants WHERE id = ${restaurantId};`, function (err: any, res: any) {
        if (err) {
          reject(err);
        }
